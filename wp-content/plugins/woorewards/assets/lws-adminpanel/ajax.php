...
class Ajax
{
	...
	public function getUsers()
	{
		...
		$sql = "SELECT ID as value, user_login as label, concat(user_login, ' &lt;<i>', user_email, '</i>&gt;') as detail FROM {$wpdb->users}";
		if( !empty($where) )
			$sql .= " WHERE " . implode(' AND ', $where);
	...
	public function getRoles()
	{
		...
		if( isset($_REQUEST['term']) )
		{
			if( $fromValue )
			{
				$term = is_array($_REQUEST['term']) ? $_REQUEST['term'] : array($_REQUEST['term']);
	...
	private function specToFilter($fields)
	{
		...
		if( isset($_REQUEST['spec']) )
		{
			...
			if( is_array($spec) )
			{
				require_once LWS_ADMIN_PANEL_PATH . '/pages/field/class-autocomplete.php';
	...
	public function googleFontList()
	{
		...
		$cached = new Cache('googlefontlist-'.($all?'all':'pop').'.json');
