...
/*
Class that will be used for new reward email in WooRewards
*/
class NewReward
{
	...
	public function settings( $settings )
	{
		...
		$settings['settings_name'] = __("New Reward", LWS_WOOREWARDS_DOMAIN);
		$settings['about']         = __("Sent to customers when they receive a new reward", LWS_WOOREWARDS_DOMAIN);
		$settings['subject']       = __("You've received a new reward", LWS_WOOREWARDS_DOMAIN);
		$settings['title']         = __("New Reward", LWS_WOOREWARDS_DOMAIN);
		$settings['header']        = __("You just got a new reward !", LWS_WOOREWARDS_DOMAIN);
		$settings['footer']        = __("Powered by WooRewards", LWS_WOOREWARDS_DOMAIN);
	...
	public function body( $html, $data, $settings )
	{
		...
		$labels = array(
			'date'   => __("Expiration Date", LWS_WOOREWARDS_DOMAIN),
			'detail' => __("Coupon Details", LWS_WOOREWARDS_DOMAIN),
			'code'   => __("Coupon Code", LWS_WOOREWARDS_DOMAIN),
			'value'  => __("Coupon Value", LWS_WOOREWARDS_DOMAIN)
		);

		if( empty($data['reward']->get_usage_limit_per_user('edit')) )
			$value .= (' ' . _x("(reusable)", "permanent coupon", LWS_WOOREWARDS_DOMAIN));
